   ...
   return new Promise((resolve, reject) => {
    ...
    reader.onload = e => {
      ...
      for(var i = 0; i < lines.length; i++){
        ...
        this.findSingleLineValue(lines[i], message, 'To:');
        this.findSingleLineValue(lines[i], message, 'From:');
        this.findSingleLineValue(lines[i], message, 'Date:');
        this.findSingleLineValue(lines[i], message, 'Message-ID:');
        this.findMultiLineValue(lines[i], message, 'Subject:');
